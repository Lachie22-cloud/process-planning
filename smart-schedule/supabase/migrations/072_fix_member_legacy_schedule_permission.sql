-- 072_fix_member_legacy_schedule_permission.sql
--
-- Root cause: migration 071 re-issued get_effective_permissions_for_user but
-- the 'member' legacy permission array omits 'batches.schedule'. This means
-- legacy member-role users (those with site_users.role='member' but no
-- tenant_user_roles entry) never receive canSchedule=true on the frontend, so
-- the drag handle is hidden and batch moves are blocked entirely — even though:
--   1. auth.has_permission() (DB/RLS level) correctly includes batches.schedule
--      for member in its legacy fallback (migration 057).
--   2. The frontend ROLE_PERMISSIONS constant also includes it for member.
--   3. The production tenant role has batches.schedule (migration 066).
--
-- The mismatch between the RPC and the DB-level function means:
--   - The DB would allow the UPDATE (RLS passes)
--   - But the frontend never attempts the move because canSchedule=false
--
-- Fix: add 'batches.schedule' to the 'member' case in both
-- get_effective_permissions_for_user and custom_access_token_hook, keeping
-- them in sync with has_permission() and the frontend ROLE_PERMISSIONS constant.
...
BEGIN
  ...
  IF v_legacy_role IS NOT NULL THEN
    ...
    -- in src/hooks/use-permissions.ts, rbac.ts, and auth.has_permission().
...
-- Also fix custom_access_token_hook to keep member permissions in sync
