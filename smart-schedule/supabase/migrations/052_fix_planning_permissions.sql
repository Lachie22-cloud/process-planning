-- 052_fix_planning_permissions.sql
--
-- Root cause: planning.tab and planning.health_check were defined in the
-- TypeScript permission constants but never seeded into the DB permissions
-- table. The legacy fallback arrays in migrations 028 and 034 also omit them,
-- so site_admin users whose effective permissions are resolved via the DB
-- (rather than the TS static fallback) never receive these permissions.
--
-- Additionally, the admin tenant role may lack planning.coverage in
-- tenant_role_permissions, and if migration 028 was not applied the legacy
-- fallback path would not run at all — causing the Planning tab to disappear.
--
-- Fixes:
--   1. Seed missing permission codes into the global catalog.
--   2. Grant all planning permissions to every site's admin tenant role.
--   3. Re-issue get_effective_permissions_for_user with complete legacy arrays.
--   4. Re-issue custom_access_token_hook with complete legacy arrays.
...
-- ============================================================
-- 2. Grant all planning permissions to every site's admin tenant role
--    (covers sites that were set up before this migration)
-- ============================================================
...
WHERE tr.code = 'admin'
  ...
  AND p.code IN (
    ...
    'planning.export',
    'planning.ai',
    'planning.health_check'
  )
ON CONFLICT (tenant_role_id, permission_id) DO NOTHING;

-- ============================================================
-- 3. Update get_effective_permissions_for_user with complete legacy arrays
-- ============================================================
...
-- ============================================================
-- 4. Update custom_access_token_hook with complete legacy arrays
-- ============================================================
